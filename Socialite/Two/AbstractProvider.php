...
use Plugins\SocialAuth\Socialite\Ports\Provider as ProviderContract;
...
abstract class AbstractProvider implements ProviderContract
{
     ...
     *
     * @var \Plugins\SocialAuth\Socialite\Two\User|null
     */
    protected $user;
     ...
     *
     * @param  array  $user
     * @return \Plugins\SocialAuth\Socialite\Two\User
     */
    ...
    public function user()
    {
        ...
        $user = $this->getUserByToken(array_get($response, 'access_token'));
    ...
    protected function userInstance(array $response, array $user)
    {
        ...
        return $this->user->setToken(array_get($response, 'access_token'))
            ->setRefreshToken(array_get($response, 'refresh_token'))
            ->setExpiresIn(array_get($response, 'expires_in'))
            ->setApprovedScopes(explode($this->scopeSeparator, array_get($response, 'scope', '')));
    }
     ...
     * @return \Plugins\SocialAuth\Socialite\Two\Token
     */
    public function refreshToken($refreshToken)
    {
        ...
        return new Token(
            ...
            array_get($response, 'refresh_token'),
